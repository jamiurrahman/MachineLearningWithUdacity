...
# print(type(finance_features[0]))
# print("First column of finance_features : ", (np.array(finance_features)[:, 0]))
# print("Second column of finance_features : ", (np.array(finance_features)[:, 1]))

# feature_salary = np.array(finance_features)[:, 0]
# feature_exercised_stock_options = np.array(finance_features)[:, 1]

# scaler = MinMaxScaler()
# rescale_feature_salary = scaler.fit(feature_salary)
# rescale_feature_exercised_stock_options = scaler.fit(feature_exercised_stock_options)

# rescaled_finance_features = np.array(np.shape(rescale_feature_salary), 2)
# rescaled_finance_features[:0] = rescale_feature_salary
# rescaled_finance_features[:1] = rescale_feature_exercised_stock_options
...
# predicting again for rescaled_finance_features
...
salary_index = np.where(salary_list == 200000)
print(np.shape(salary_index))
print(type(salary_index))
print((salary_index[0]))
print((salary_index))


salary_list2 = (np.array(finance_features)[:, 1])
salary_index2 = np.where(salary_list2 == 100000)
print(np.shape(salary_index2))
print(type(salary_index2))
print((salary_index2[0]))
print((salary_index2))